#!/usr/bin/env python3
"""
2D lid-driven cavity (Oseen / explicit convection) in Firedrake
...
Examples:
  Serial:
    ...
    python cavity_2d_oseen_mg.py --base 16 --levels 3 --Re 100 --cfl 0.5 --steps 200 -ksp_converged_reason

  16 MPI ranks:
    export OMP_NUM_THREADS=1
    mpiexec -n 16 python cavity_2d_oseen_mg.py --base 16 --levels 3 --Re 100 --cfl 0.5 --steps 200 -ksp_monitor -ksp_converged_reason
"""
...
parser = argparse.ArgumentParser(
    description="2D lid-driven cavity (Oseen) with SUPG/PSPG and matfree geometric MG"
)

# Mesh controls (geometric MG needs a hierarchy)
parser.add_argument("--base", type=int, default=16, help="Coarsest mesh cells/dir (UnitSquareMesh(base,base))")
parser.add_argument("--levels", type=int, default=3, help="Uniform refinements; finest = base * 2^levels")
...
parser.add_argument("--pc", choices=["mg", "gamg"], default="mg",
                    help="Preconditioner. 'mg' = geometric multigrid (needs hierarchy). 'gamg' = PETSc GAMG (assembled).")
...
# Output
parser.add_argument("--write-freq", type=int, default=10, help="VTK write frequency (in steps)")
parser.add_argument("--vtk", type=str, default="cavity.pvd", help="Output PVD filename")
...
def main():
    ...
    base_mesh = UnitSquareMesh(args.base, args.base, distribution_parameters=dparams)
    ...
    # ---- BCs (UnitSquareMesh boundary ids: 1 left, 2 right, 3 bottom, 4 top) ----
